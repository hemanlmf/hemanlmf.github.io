...
# Answer
When $(a,b) = (0,1)$, we generate $U \sim U(0,1)$ as the underlying random variable in standard Monte Carlo since the support of $U$ is same as interval of integration. The idea is essentially
$$
\int_0^1 g(x) dx = \int_0^1 g(u) f(u) du = E[g(U)] 
$$
since the pdf of $U$ is $f(u)=1$  

Similarly, for $\theta = \int_a^b g(x) dx$, we generate $V \sim U(a,b)$. Then
$$
\int_a^b g(x) dx = (b-a)\int_a^b g(v) \frac{1}{b-a} dv = (b-a)E[g(V)] 
$$
