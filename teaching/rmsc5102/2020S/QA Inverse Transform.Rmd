...
# Answer
Let $v=f(x)$ be a general pdf and $w=F(x)=\int_{-\infty}^{\infty} f(x) dx$ be its cdf. Assume their inverse both exists, then we have $x=f^{-1}(v)$ and $x=F^{-1}(w)$  
...
However, for cdf, $W \sim Unif(0,1)$ according to the [probability integral transform](https://en.wikipedia.org/wiki/Probability_integral_transform). As we know how to generate $Unif(0,1)$, we use cdf for inverse transform.  
...
\end{aligned}
$$
Let $V=f(X)=2e^{-2X}$ and $W=F(X)=1-e^{-2X}$  
Then $X=-\frac{1}{2} ln(\frac{1}{2} V)$ and $X=-\frac{1}{2} ln(1-W)$  
We use histogram to visualize the distribution of them:
```{r answer}
